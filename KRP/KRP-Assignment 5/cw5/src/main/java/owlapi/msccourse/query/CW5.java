	...
	public Set<QueryResult> performQuery(OWLClassExpression exp, QueryType type) {
		...
		case EQUIVALENTCLASSES:
			...
			for (OWLClass eq: eqcs.getEntities()) {
				if (!eq.toString().equals("owl:Nothing") && !r.getEquivalentClasses(eq).contains(df.getOWLNothing()) && !results.contains(new QueryResult(eq,true,QueryType.EQUIVALENTCLASSES))) {
		...
		while (true) {
			...
			for (OWLClassExpression exp1 : restrictions) {
				for (OWLClassExpression exp2 : restrictions) {
					...
					if (!addRestrictions.contains(exp3)) {
