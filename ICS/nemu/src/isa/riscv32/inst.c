...
extern void insert_bt(uint64_t addr, uint8_t type, uint64_t goal_pc, uint64_t instruction);
...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (SEXT(BITS(i, 7, 7), 1) << 11) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 1); } while(0)

static inline void csrrs_instrction(int rd, word_t imm, word_t src)
{
	word_t t, *ptr = NULL;
...
static inline void csrrw_instrction(int rd, word_t imm, word_t src)
{
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm ? 1 : 0);//fib over/bit lose  
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I, csrrs_instrction(rd, imm, src1));
	INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, csrrw_instrction(rd, imm, src1));
