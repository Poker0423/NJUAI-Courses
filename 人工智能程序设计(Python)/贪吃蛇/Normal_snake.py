...
def Show_Start_Interface():
	...
	title_content = title_Font.render('贪吃蛇', True, (255, 255, 255), (128,0,128))
	angle = 0
	while True:
	...
	while True:
		for event in pygame.event.get():
			if event.type == QUIT:
				...
				if (event.key in (K_a, K_LEFT)) and (direction != 'right'):
					direction = 'left'
				elif (event.key in (K_d, K_RIGHT)) and (direction != 'left'):
					direction = 'right'
				elif (event.key in (K_w, K_UP)) and (direction != 'down'):
					direction = 'up'
				elif (event.key in (K_s, K_DOWN)) and (direction != 'up'):
					direction = 'down'
