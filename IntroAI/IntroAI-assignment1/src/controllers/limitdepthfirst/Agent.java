...
public class Agent extends AbstractPlayer {
    ...
    double bestScore = 10000;                                            //已走过的路径的最优评分（值越小就越好）
    boolean hasKey = false;                                              //是否已经拿到钥匙
    int singleSearchDepth = 0;                                           //当前搜索的深度
    int searchDepth = 5;                                                 //规定的受限搜索深度（综合搜索时间和准确性而确定）
    ...
    private double distance(StateObservation stateObs){                 //利用目标的位置和钥匙的位置尝试构造启发式函数distance
    ...
    private void getLimitDepthFirstActions(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        pastState.add(stateObs); // 将当前状态加入已走过的状态中
        singleSearchDepth++; // 搜索深度+1
        for (Types.ACTIONS action : stateObs.getAvailableActions()) { // 尝试当前局面所有可能的动作
            ...
            if (singleSearchDepth == searchDepth) { // 如果当前搜索的深度等于受限的深度
                ...
                return;
            } else if (equalTest(stCopy) || stCopy.isGameOver()) { // 如果动作施加后的状态之前已经到达过或者游戏失败，不进行操作
