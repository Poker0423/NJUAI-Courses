...
public class Agent extends AbstractPlayer {
    ...
    ArrayList<Types.ACTIONS> Actions = new ArrayList<Types.ACTIONS>();           //存储已走过的动作
    ArrayList<StateObservation> pastState = new ArrayList<StateObservation>();      //存储已走过的状态
    ...
    private boolean equalTest(StateObservation state) {
        return pastState.stream()
                        .anyMatch(so -> state.equalPosition(so));        //流式操作，判断当前状态之前是否到达过
    }


    private boolean getDepthFirstActions(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        pastState.add(stateObs);                                                //将当前状态加入已走过的状态
        for (Types.ACTIONS action : stateObs.getAvailableActions()) {             //尝试当前局面所有可以的动作
            StateObservation stCopy = stateObs.copy();                          //创建一个当前状态的副本，用于模拟施加动作
        ...
        pastState.remove(pastState.size() - 1);                          //当前局面没有办法成功，则删除
        return false;
    }
    ...
    private void printDebug(ArrayList<Observation>[] positions, String str) {
        if (positions != null) {
            ...
            sb.append(str).append(":").append(positions.length).append("(");
            for (ArrayList<Observation> position : positions) {
                sb.append(position.size()).append(",");
            }
            sb.append("); ");
